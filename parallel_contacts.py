...
def worker(input_queue: queue.Queue, output_queue: queue.Queue, worker_id: int):
    ...
    while True:
        ...
        print(f"Worker {worker_id} processing chunk with {len(timepoint_chunk)} timepoints: {list(timepoint_chunk)}")
...
def process_chunk(timepoint_chunk, arr_segments, contact_length, df_sum, arrested, time_interval):
    print(f"[DEBUG] Processing chunk with timepoints: {list(timepoint_chunk)}")
    is_in_chunk = np.isin(arr_segments[:, 1], timepoint_chunk)
    arr_segments_chunk = arr_segments[is_in_chunk]
    print(f"[DEBUG] arr_segments_chunk shape: {arr_segments_chunk.shape}")

    if arr_segments_chunk.size == 0:
        print("[DEBUG] No segments found for this chunk.")
        return None, None, None

    unique_objects_chunk = np.unique(arr_segments_chunk[:, 0])
    print(f"[DEBUG] unique_objects_chunk: {unique_objects_chunk}")

    print(f"[DEBUG] Calling contacts() with {len(unique_objects_chunk)} unique object IDs and contact_length: {contact_length}")
    df_cont = contacts(unique_objects_chunk, arr_segments_chunk, contact_length)
    if not df_cont or all(len(df) == 0 for df in df_cont):
        print("[DEBUG] contacts() returned no data for this chunk.")
        return None, None, None
    ...
    if not df_no_daughter_func or all(len(df) == 0 for df in df_no_daughter_func):
        print("[DEBUG] no_daughter_contacts() returned no data for this chunk.")
        df_no_daughter = pd.DataFrame()
    else:
        df_no_daughter = pd.concat(df_no_daughter_func, ignore_index=True)
        print(f"[DEBUG] no_daughter_contacts() processed {len(df_no_daughter_func)} chunks.")
    ...
    if not df_alive or all(len(df) == 0 for df in df_alive):
        print("[DEBUG] contacts_moving() returned no alive contacts for this chunk.")
        df_no_dead_ = pd.DataFrame()
    else:
        df_no_dead_ = pd.concat(df_alive, ignore_index=True)
        print(f"[DEBUG] contacts_moving() returned {len(df_alive)} alive contacts chunks.")

    print("[DEBUG] Finished processing chunk.")
...
def main(timepoints, arr_segments, contact_length, df_sum, arrested, time_interval):
    ...
    max_processes = max(1, min(61, multiprocessing.cpu_count() - 1))
    ...
    for chunk in timepoint_chunks:
        print(f"Enqueueing chunk with {len(chunk)} timepoints: {list(chunk)}")
    ...
    while len(results) < tasks_count:
        try:
            ...
            print("Collected result from output queue.")
        except queue.Empty:
            time.sleep(0.1)
