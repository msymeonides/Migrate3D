...
dpg.create_context()
parameters = {'Interval': 15, 'arrest_displacement': 3.0, 'contact_length': 10, 'arrested': 0.95, 'moving': 4,
              ...
              'category_col': 'Code', 'pca_format': None, 'Contact': False, 'Tau_val': 6, 'infile_tracks': False,
              'multi_track': False, 'two_dem': False, 'interpolate': False}
...
with dpg.window(label="Migrate3D", width=1000, height=600) as Window:
    ...
    pca_filter = dpg.add_input_text(width=200,
                                    label='If you would like to filter cell types for PCA enter them here separated'
                                          ' by a comma( , ).',
                                    callback=input_return, tag='pca_filter')

    multi_check = dpg.add_checkbox(label='Adjust for Multi-tracked time points?', callback=formatting_check,
                                   tag='multi_track')

    two_dem_check = dpg.add_checkbox(label='Adjust for two-dimensional data? check if using 2D data or '
                                           'you want to convert your data to two dimensions',
                                     callback=formatting_check, tag='two_dem')

    interpolate_check = dpg.add_checkbox(label='Lazy Interpolation for missing data points?', callback=formatting_check,
                                         tag='interpolate')
