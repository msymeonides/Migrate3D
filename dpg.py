...
parameters = {'Interval': 15, 'arrest_displacement': 3.0, 'contact_length': 10.0, 'arrested': 0.95, 'moving': 4,
              ...
              'x_for': 'X Coordinate', 'y_for': 'Y Coordinate', 'z_for': 'Z Coordinate', 'Contact': True}
...
def migrate3D(param):
        ...
        try:
            ...
            while p_bar_increase < 1:
                ...
                if format_check == 'y':
                    ...
                    df = df.sort_values(by=[df.columns[sort_1 - 1], df.columns[sort_2 - 1]], ascending=True)
                ...
                list_of_df = []
                for cell in cell_id:
                    x_values = list(df.loc[df[parent_id] == cell, x_for])
                    y_values = list(df.loc[df[parent_id] == cell, y_for])
                    z_values = list(df.loc[df[parent_id] == cell, z_for])
                    time = list(df.loc[df[parent_id] == cell, time_col])
                ...
                cols_angles = cols[(-time_points_odd):]
                cols_euclidean = cols[9:-time_points_odd * 2 - 1]
                ...
                p_bar_increase += 0.25
                dpg.set_value('pbar', p_bar_increase)
                df_sum, time_interval = summary_sheet(df_calc, cell_id, cols_angles, cols_euclidean, parent_id)
                p_bar_increase += 0.25
                dpg.set_value('pbar', p_bar_increase)
                df_cont = contacts(cell_id, df, parent_id, x_for, y_for, z_for, time_col)
                if len(df_cont) == 0 or contact_parameter is False:
                    pass
                else:
    ...
    def summary_sheet(df, cell_id, cols_angles, cols_euclidean, parent_id):
        sum_ = {}
        ...
        for cell in cell_id:
            ...
            overall_euclidean_median, overall_angle_median = overall_medians(cell, df, cols_angles, cols_euclidean,
                                                                             parent_id)
            ...
            sum_[cell] = cell, duration, final_euclid, max_euclid, max_path, straightness, tc_straightness, \
                         ...
                         arrest_coefficient, overall_angle_median, overall_euclidean_median
        ...
        df_sum.columns = ['Cell ID', 'Duration', 'Final Euclidean', 'Max Euclidean',
                          ...
                          'Arrest Coefficient',
                          'Overall Angle Median', 'Overall Euclidean Median']
        return df_sum, time_interval
    ...
    def overall_medians(cell, df, cols_angles, cols_euclidean, parent_id):
        ...
        for col_ in cols_angles:
            ...
            angle_median = [x for x in angle_median if np.isnan(x) == False or x is None]
        ...
        for cols_ in cols_euclidean:
            ...
            euclidean_median = [x for x in euclidean_median if np.isnan(x) == False or x is None]
        ...
        return overall_euclidean_median, overall_angle_median
...
with dpg.window(label="Migrate3D", width=800, height=600) as Window:
    ...
    timelapse = dpg.add_input_float(width=100,
                                    label='Timelapse interval in seconds',
    ...
    dpg.add_checkbox(label='Click to run Contact3D on Dataset', callback=run_contact)
